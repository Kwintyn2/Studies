...
def yayinevi_guncelle(eski_yayinevi,yeni_yayinevi):
    imlec.execute("UPDATE kitaplik SET Yayınevi = ? WHERE Yayınevi = ?", (yeni_yayinevi,eski_yayinevi))
    sq_baglanti.commit()

def yazar_silme(silinecek):
    imlec.execute("DELETE FROM kitaplik where yazar = ?", (silinecek,))
    sq_baglanti.commit()
